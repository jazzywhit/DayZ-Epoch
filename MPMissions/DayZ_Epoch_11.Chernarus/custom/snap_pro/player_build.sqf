...
if (_isPole) then {
    ...
    _nearestViolation = nearestLocations [_playerPos, ["NameCityCapital", "Airport","NameCity","NameVillage"],250];
    if (!_stopBuild && count _nearestViolation > 0) then { _stopBuild = true; _stopBuildMsg = "You cannot build a plot pole within 250m of a Capital, Airport, City, or Village!";};
    ...
    _buildOk = ["Land_Misc_deerstand"];
    {
        if (!(typeOf _x in _buildOk)) then {
            if (!_stopBuild && (["Land_",(typeOf _x),false] call fnc_inString)) then { _stopBuild = true; _stopBuildMsg = "You cannot build a plot pole within 50m of a Building!";};
        };
    } forEach nearestObjects [_playerPos, ["House"], 50];
};
