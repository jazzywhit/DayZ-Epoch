...
_stopBuildMsg = "";
if (_isPole) then {
    private ["_nearestViolation", "_playerPos", "_buildOk"];
    _playerPos = getPosATL player;

    _nearestViolation = nearestLocations [_playerPos, ["NameCityCapital", "Airport"],1000];
    if (!_stopBuild && count _nearestViolation > 0) then { _stopBuild = true; _stopBuildMsg = "You cannot build a plot pole within 1000m of a Capital or Airport!";};

    _nearestViolation = nearestLocations [_playerPos, ["NameCity"],500];
    if (!_stopBuild && count _nearestViolation > 0) then { _stopBuild = true; _stopBuildMsg = "You cannot build a plot pole within 500m of a City!";};

    _nearestViolation = nearestLocations [_playerPos, ["NameVillage"],250];
    if (!_stopBuild && count _nearestViolation > 0) then { _stopBuild = true; _stopBuildMsg = "You cannot build a plot pole within 250m of a Village!";};
    ...
    _buildOk = ["Land_Misc_deerstand"];
    {
        if (!(typeOf _x in _buildOk)) then {
            if (!_stopBuild && (["Land_",(typeOf _x),false] call fnc_inString)) then { _stopBuild = true; _stopBuildMsg = "You cannot build a plot pole within 100m of a Building!";};
        };
