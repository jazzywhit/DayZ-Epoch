...
while {true} do {
	...
	if (_inRange) then {
		if (!notified) then {
			cutText ['Mining Area: To mine you must equip a Sledgehammer or Crowbar, then select "Start Mining" from your scroll menu. Sledgehammers work much better', 'PLAIN'];
			notified = true;
		};
	} else { notified = false; };
