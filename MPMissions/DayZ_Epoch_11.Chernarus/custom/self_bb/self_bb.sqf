...
// CONFIG START																																						\\
//------------------------------------------------------------------------------------------------------------------------------------------------------------------\\
_useTime = 300; //Time in seconds before being able to use again.
...
_infectionChance =50; //Chance of getting infected while using the bloodbag (0 = disabled, 40 = 40%, 100 = 100% etc)
...
if (_hospitalEnable == 1)  then { // if Hospital enabled
    ...
    if (_hospitalFound > 0 && _decke) then {
        _nearHospital = true;
        _bloodMultiplier = 3; // multiplier to Bloodamount
        _infectionMultiplier = .01; //multiplier to Infectionchance
    };
}; 
if (_ambulanceEnable == 1)  then {
    ...
    if (_ambulanceFound > 0 ) then {
		...
		_infectionMultiplier = .50; //multiplier to Infectionchance
    }; 
};
